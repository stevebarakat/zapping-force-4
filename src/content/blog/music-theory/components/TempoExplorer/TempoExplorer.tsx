...
import "./TimeSignatureExplorer.css";
...
const TimeSignatureExplorer = () => {
  ...
  const getTempoLabel = (
    ...
    if (bpm <= 20)
      return {
        term: "Larghissimo",
        description: "Extremely slow (20 BPM and below)",
      };
    if (bpm <= 40)
      return { term: "Grave", description: "Slow and solemn (20-40 BPM)" };
    if (bpm <= 60)
      return { term: "Lento/Largo", description: "Slowly/Broadly (40-60 BPM)" };
    if (bpm <= 66)
      return { term: "Larghetto", description: "Rather broadly (60-66 BPM)" };
    if (bpm <= 76)
      return { term: "Adagio", description: "Slow and stately (66-76 BPM)" };
    if (bpm <= 80)
      return { term: "Adagietto", description: "Rather slow (70-80 BPM)" };
    if (bpm <= 108)
      return {
        term: "Andante/Andantino",
        description:
          "At a walking pace/Slightly faster than andante (76-108 BPM)",
      };
    if (bpm <= 120)
      return {
        term: "Moderato/Allegretto",
        description: "Moderately/Moderately fast (108-120 BPM)",
      };
    if (bpm <= 168)
      return {
        term: "Allegro/Vivace",
        description: "Fast and bright/Lively and fast (120-168 BPM)",
      };
    if (bpm <= 200)
      return { term: "Presto", description: "Very fast (168-200 BPM)" };
    return { term: "Prestissimo", description: "Extremely fast (200+ BPM)" };
  };
  ...
  useEffect(() => {
    ...
    clickHighRef.current = new Tone.Player({
      url: "/audio/metronome/high.mp3",
      volume: -10,
    }).toDestination();

    clickMidRef.current = new Tone.Player({
      url: "/audio/metronome/mid.mp3",
      volume: -10,
    }).toDestination();

    clickLowRef.current = new Tone.Player({
      url: "/audio/metronome/low.mp3",
      volume: -10,
    }).toDestination();
    ...
    <div className="time-signature-explorer">
      ...
      <div className="tempo-control">
        ...
        <span className="tempo-value">
          {tempo} BPM: {getTempoLabel(tempo).term} -{" "}
          {getTempoLabel(tempo).description}
        </span>
        <Slider
          id="tempo-slider"
          min={40}
